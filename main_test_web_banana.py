...
try:
    while True:
        ...
        if GPIO.input(BUTTON_PIN) == GPIO.LOW:
            ...
            print(f"🍌 Predicted Ripeness: {pred_name}")
...
class EfficientFeatureExtractor(nn.Module):
    def __init__(self, pretrained=False):
        super().__init__()
        self.base = (
            EfficientNet.from_pretrained('efficientnet-b3') if pretrained
            else EfficientNet.from_name('efficientnet-b3')
        )
    def forward(self, x):
        features = self.base.extract_features(x)
...
def load_model():
    cnn_extractor = EfficientFeatureExtractor(pretrained=False).to(DEVICE)
    cnn_extractor.base.load_state_dict(torch.load("Early_Fusion_Banana/Banana_CNN.pth", map_location=DEVICE), strict=False)
    cnn_extractor.eval()
    sensor_feat = SensorNetFeat().to(DEVICE)
    sensor_feat.load_state_dict(torch.load("Early_Fusion_Banana/banana_900_sensor_modelOct.pth", map_location=DEVICE), strict=False)
    sensor_feat.eval()
    img_dim = cnn_extractor(torch.randn(1, 3, 224, 224)).shape[1]
    sensor_dim = sensor_feat(torch.randn(1, 4)).shape[1]
    fusion_model = EarlyFusionModel(cnn_extractor, sensor_feat, img_dim, sensor_dim, NUM_CLASSES).to(DEVICE)
    fusion_model.load_state_dict(torch.load("Early_Fusion_Banana/banana_early_fusion_model_October.pth", map_location=DEVICE))
    fusion_model.eval()
    return fusion_model
...
scaler = load_scaler()
label_encoder = load_label_encoder()
regression_model = joblib.load("Early_Fusion_Banana/ripeness_regression_model_new2.pkl")
...
def predict_ripeness(image, sensor_values):
    ...
    with torch.no_grad(): pred_idx = fusion_model(img, sensor_tensor).argmax(dim=1).item()
    return pred_idx
...
if __name__=="__main__":
    result = run_analysis()
    if result:
        print(f"📸 Image: {f'/home/device/ML_model/image_{int(time.time())}.jpg'}")
