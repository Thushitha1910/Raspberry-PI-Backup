...
def read_sensors(sample_id="banana1"):
    try:
        ...
        print(f"[{sample_id}] MQ4: {round(ppm_mq4, 2)} ppm | MQ135: {round(ppm_mq135, 2)} ppm | TGS2602: {round(ppm_tgs2602, 2)} ppm | NIR: {violet}")
...
try:
    while True:
        ...
        if GPIO.input(BUTTON_PIN) == GPIO.LOW:
            ...
            if len(boxes) == 0:
                print("⚠️ No objects detected by YOLO. Using full image instead.")
                cropped_image = image
            else:
