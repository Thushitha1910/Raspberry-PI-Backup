...
def load_model():
    ...
    cnn_extractor.base.load_state_dict(torch.load("Early_Fusion_Bellpepper/best_bellpepper_model.pth", map_location=DEVICE), strict=False)
    cnn_extractor.eval()
    sensor_feat = SensorNetFeat().to(DEVICE)
    sensor_feat.load_state_dict(torch.load("Early_Fusion_Bellpepper/bellpepper_900_sensor_model.pth", map_location=DEVICE), strict=False)
    sensor_feat.eval()
    ...
    fusion_model.load_state_dict(torch.load("Early_Fusion_Bellpepper/bellpepper_early_fusion_model_November.pth", map_location=DEVICE))
    fusion_model.eval()
    return fusion_model

def load_scaler():
    return joblib.load('Early_Fusion_Bellpepper/bellpepper_sensor_scaler.save')

def load_label_encoder():
    return joblib.load('Early_Fusion_Bellpepper/bellpepper_label_encoder.save')

def load_regression_model():
    return joblib.load("Early_Fusion_Bellpepper/bellpepper_regression_model.pkl")
...
# === Read Sensors ===
def read_sensors(sample_id="bellpepper1"):
    try:
        # MQ4
        ...
        while not as7263.data_ready:
            time.sleep(0.05)
        violet = as7263.violet+nir_offset
        as7263.driver_led = False
...
def run_analysis():
    ...
    sensor_values = read_sensors("bellpepper1")
