...
def load_model():
    ...
    cnn_extractor.base.load_state_dict(
        torch.load("Early_Fusion_Carrot/best_carrot_model.pth", map_location=DEVICE),
        strict=False
    )
    cnn_extractor.eval()
    ...
    sensor_feat.load_state_dict(
        torch.load("Early_Fusion_Carrot/carrot_900_sensor_model.pth", map_location=DEVICE),
        strict=False
    )
    sensor_feat.eval()
    ...
    fusion_model.load_state_dict(
        torch.load("Early_Fusion_Carrot/carrot_early_fusion_model_October.pth", map_location=DEVICE)
    )
    fusion_model.eval()
    return fusion_model
...
def load_scaler():
    return joblib.load('Early_Fusion_Carrot/carrot_sensor_scaler.save')


def load_label_encoder():
    return joblib.load('Early_Fusion_Carrot/label_encoder_October.save')
...
def load_regression_model():
    return joblib.load("Early_Fusion_Carrot/carrot_regression_model_new.pkl")
...
print("\n🔹 System ready. Press the button to start analysis.\n")
time.sleep(3)
...
try:
    while True:
        ...
        if GPIO.input(BUTTON_PIN) == GPIO.LOW:
            print("🔘 Button pressed! Starting analysis...\n")
            time.sleep(1)
            ...
            sensor_values = read_sensors("carrot1")
            ...
            print(f"🍌 Predicted Freshness: {pred_name}")
            print(f"🕒 Days to Rotten: {predicted_days}")
            print(f"📅 Estimated Rotten Date: {rotten_date_str}\n")
...
def load_model():
    cnn_extractor = EfficientFeatureExtractor(pretrained=False).to(DEVICE)
    cnn_extractor.base.load_state_dict(torch.load("Early_Fusion_Carrot/best_carrot_model.pth", map_location=DEVICE), strict=False)
    cnn_extractor.eval()
    sensor_feat = SensorNetFeat().to(DEVICE)
    sensor_feat.load_state_dict(torch.load("Early_Fusion_Carrot/carrot_900_sensor_model.pth", map_location=DEVICE), strict=False)
    sensor_feat.eval()
    img_dim = cnn_extractor(torch.randn(1, 3, 224, 224)).shape[1]
    sensor_dim = sensor_feat(torch.randn(1, 4)).shape[1]
    fusion_model = EarlyFusionModel(cnn_extractor, sensor_feat, img_dim, sensor_dim, NUM_CLASSES).to(DEVICE)
    fusion_model.load_state_dict(torch.load("Early_Fusion_Carrot/carrot_early_fusion_model_October.pth", map_location=DEVICE))
    fusion_model.eval()
    return fusion_model
