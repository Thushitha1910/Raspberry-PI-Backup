...
def load_model():
    ...
    cnn_extractor.base.load_state_dict(torch.load("Early_Fusion_Potato/best_potato_model.pth", map_location=DEVICE), strict=False)
    cnn_extractor.eval()
    sensor_feat = SensorNetFeat().to(DEVICE)
    sensor_feat.load_state_dict(torch.load("Early_Fusion_Potato/potato_900_sensor_model.pth", map_location=DEVICE), strict=False)
    sensor_feat.eval()
    ...
    fusion_model.load_state_dict(torch.load("Early_Fusion_Potato/potato_early_fusion_model_November.pth", map_location=DEVICE))
    fusion_model.eval()
    return fusion_model

def load_scaler():
    return joblib.load('Early_Fusion_Potato/potato_sensor_scaler.save')

def load_label_encoder():
    return joblib.load('Early_Fusion_Potato/label_encoder_potato.save')

def load_regression_model():
    return joblib.load("Early_Fusion_Potato/potato_regression_model.pkl")
...
# === Read Sensors ===
def read_sensors(sample_id="potato1"):
    try:
        # MQ4
...
def run_analysis():
    ...
    sensor_values = read_sensors("potato1")
