...
def load_model():
    ...
    cnn_extractor.base.load_state_dict(torch.load("Early_Fusion_Strawberry/best_strawberry_model.pth", map_location=DEVICE), strict=False)
    cnn_extractor.eval()
    sensor_feat = SensorNetFeat().to(DEVICE)
    sensor_feat.load_state_dict(torch.load("Early_Fusion_Strawberry/strawberry_900_sensor_model.pth", map_location=DEVICE), strict=False)
    sensor_feat.eval()
    ...
    fusion_model.load_state_dict(torch.load("Early_Fusion_Strawberry/strawberry_early_fusion_model_October.pth", map_location=DEVICE))
    fusion_model.eval()
    return fusion_model

def load_scaler():
    return joblib.load('Early_Fusion_Strawberry/strawberry_sensor_scaler.save')

def load_label_encoder():
    return joblib.load('Early_Fusion_Strawberry/strawberry_label_encoder.save')

def load_regression_model():
    return joblib.load("Early_Fusion_Strawberry/strawberry_regression_model.pkl")
